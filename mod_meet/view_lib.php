...
function meet_render_recording_view_page($course, $cm, $context, $recordingid) {
    ...
    // Iframe
    $output .= '<div id="cooke_warning" class="hidden alert alert-danger">'.get_string('cookie_warning', 'meet');
    $output .= '<br /><a target="_BLANK" href="'.$CFG->wwwroot.'/mod/meet/help.php">Mais ajuda</a> </div>';
...
function meet_files($context){
    $fs = get_file_storage();
    
    $files = $fs->get_area_files($context->id, 'mod_meet', 'content', 0);
    $file = null;
    ...
    foreach($files as $file){
        ...
        $o .= "<a href='$url'>{$file->get_filename()}</a> <br />";
    }

    if($o){
        $o = "<h3>".get_string('aditional_files', 'meet')."</h3>" . meet_render_spacer(). $o;
    }
...
function meet_render_availability($meet, $cm, $context) {
    ...
    if(meet_has_capability('addinstance', $context)){
        $output .= "<div class='alert alert-danger'> ATENÇÃO: Junte-se a reunião com o seu email @universo.univates para poder iniciar a gravação da aula! </div>";
    } else if($available){
        $output .= "<div class='alert alert-danger'> ATENÇÃO: Para participar da videoconferência, junte-se a reunião com o seu e-mail @universo.univates. </div>";
    }
...
function meet_cookie_checker(){
    $content = "<script>";
    $content .= "    let allowed = 'true';";
    $content .= "    var receiveMessage = function (evt) { if (evt.data === 'MM:3PCunsupported') { allowed = 'false'; } };";
    $content .= "    window.addEventListener('message', receiveMessage, false);";
    $content .= "    window.addEventListener('load', function () {";
    $content .= "       let testReq = new Request( 'https://googleads.g.doubleclick.net/pagead/id', { method: 'HEAD', mode: 'no-cors' } );";
    $content .= "       fetch(testReq).then(response => { }).catch(error => { allowed = 'false'; }).then(response => { if(allowed == 'false') { var warn = document.getElementById('cooke_warning'); warn.classList.remove('hidden'); } });";
    $content .= "    });";
    $content .= "</script>";
    $content .= "<iframe src='https://mindmup.github.io/3rdpartycookiecheck/start.html' style='display: none;'></iframe>";
    return $content;
}
