...
function meet_add_instance($data, $mform) {
    ...
    file_save_draft_area_files($draftitemid, $context->id, 'mod_meet', 'content', 0, $options);
...
function meet_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    $fs = get_file_storage();
    $file = $fs->get_file($context->id, 'mod_meet', $filearea, $itemid, $filepath, $filename);
    if (!$file) {
...
function meet_save_participants(&$data) {
    ...
    // Check if the user can view the resource. Remove him otherwise
    if( $data->instance ){
        $modinfo = get_fast_modinfo( $data->course );
        $cm = $modinfo->get_cm( $data->coursemodule );
        foreach($users as $key => $value){
            if(!\core_availability\info_module::is_user_visible($cm, $value) ){
                unset( $users[$key]);
            }
        }
    }else{
        // Do not invite on the event creation
        return;
    }
...
function meet_add_user_to_event($meetorid, $userorid) {
    ...
    $mod = $DB->get_record_sql( "SELECT * FROM {course_modules} WHERE instance = ".$meetorid->id." AND module = (SELECT id FROM {modules} WHERE name = 'meet')" );

    if($mod){
        $modinfo = get_fast_modinfo( $mod->course );
        $cm = $modinfo->get_cm( $mod->id );

        if(!\core_availability\info_module::is_user_visible($cm, $userorid->id)){
            return;
        }
    }
...
/**
 * Refesh the users on this event. Used on the create hook
 *
 * @param int|stdClass $instance Meet module instance or ID.
 */
function meet_refresh_users($meetid = null){
    global $DB;
    ...
    $data->gclient = meet_create_google_client($data->config);
    $data->gcalendarservice = meet_create_google_calendar_service($data->gclient);
    $data->gevent = meet_get_google_calendar_event($data->gcalendarservice, $data->config->calendarid, $meet->geventid);
...
function meet_is_meeting_room_available($meet) {
    ...
    $role = meet_get_user_role_on_course($meet);

    // Is open 
    if(
        ($timenow > ( $timestart - 300 ) && $timenow < $timeend) || 
        $role == 'editingteacher' || // Teachers can access at all times 
        $role == 'tutor' || // Tutors can access at all times
        is_siteadmin() //Site admins can access at all times
    ){
        return true;
    }
...
/**
 * Get the user role on the course of the meet 
 *
 * @param  meet
 * @return rolename 
 */
function meet_get_user_role_on_course($meet){
    global $DB, $USER;

    $sql = "SELECT mro.id, mro.shortname 
            FROM 
            ...
            WHERE mra.contextid = ( 
                    ...
                    WHERE 
                        ...
                        instanceid = ?) AND 
                  mra.userid = ? AND 
                  mra.roleid = mro.id ORDER BY id;";
    return $DB->get_record_sql($sql, [$meet->course, $USER->id] )->shortname;
